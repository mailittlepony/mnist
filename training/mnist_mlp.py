...
class Model:
    def __init__(self, width=512, depth=2, activation="silu", dropout=0.0):
        act = Tensor.silu if activation.lower() == "silu" else Tensor.relu
        layers: list[Callable[[Tensor], Tensor]] = [lambda x: x.flatten(1)]
        in_features = 784
        for _ in range(depth):
            layers += [nn.Linear(in_features, width), act]
            if dropout and dropout > 0:
                layers += [lambda x, p=dropout: x.dropout(p)]
            in_features = width
        layers += [nn.Linear(in_features, 10)]
        self.layers = layers
...
if __name__ == "__main__":
    ...
    DROPOUT = float(getenv("DROPOUT", 0.1))
    ...
    model = Model(width=WIDTH, depth=DEPTH, activation=ACT, dropout=DROPOUT)
